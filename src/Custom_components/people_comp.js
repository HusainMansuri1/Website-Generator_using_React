      ...
      <section className='people'>
        ...
        <div className='persons-wrap'>
          <ul className='gf-block'>
            ...
            {props.pushPeopleState.map((peop, ind) => (
              <People key={'gf-no-' + peop.id} onDeletee={() => props.delete(ind, peop.name)} name={peop.name} age={peop.age} change={(event) => this.nameChangeHandler(event, peop.id)} current={peop} />
            ))}
          </ul>
